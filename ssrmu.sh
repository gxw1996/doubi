...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/gxw996/hero/master/bbr.sh; then
...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
	SSQRcode="http://kistom.com/qr/index.html?q=${SSurl}"
...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: myself):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="myself"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 41520):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="41520"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: ):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="5Jq@yvu[UdLv6N*WHJHKHK@FDA%"
...
Download_SSR(){
	cd "/usr/local"
	wget -N --no-check-certificate "https://github.com/gxw996/shadowsocksr/archive/manyuser.zip"
	#git config --global http.sslVerify false
	#env GIT_SSL_NO_VERIFY=true git clone -b manyuser https://github.com/gxw996/shadowsocksr.git
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/gxw996/hero/master/other/ssrmu_centos -O /etc/init.d/ssrmu; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/gxw996/hero/master/other/ssrmu_debian -O /etc/init.d/ssrmu; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssrmu
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/gxw996/hero/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh banall
	rm -rf ban_iptables.sh
}
# 解封 BT PT SPAM
UnBanBTPTSPAM(){
	wget -N --no-check-certificate https://raw.githubusercontent.com/gxw996/hero/master/ban_iptables.sh && chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -rf ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	...
	[[ -z ${sh_new_ver} ]] && sh_new_ver=$(wget --no-check-certificate -qO- "https://raw.githubusercontent.com/gxw996/hero/master/ssrmu.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	if [[ ${sh_new_ver} != ${sh_ver} ]]; then
		...
		if [[ ${yn} == [Yy] ]]; then
			...
			if [[ $sh_new_type == "softs" ]]; then
				...
				wget -N --no-check-certificate https://raw.githubusercontent.com/gxw996/hero/master/ssrmu.sh && chmod +x ssrmu.sh
			fi
